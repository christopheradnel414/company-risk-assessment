...
class JobManager:
    ...
    def complete_job(self, job_id: str, result: AssessmentResult) -> None:
        job = self._jobs.get(job_id)
        if job:
    ...
    def set_job_ambiguous(self, job_id: str, candidates: List[CandidateCompany]) -> None:
        job = self._jobs.get(job_id)
        if job:
    ...
    def set_resolved_context(self, job_id: str, context: SearchContext) -> None:
        job = self._jobs.get(job_id)
        if job:
            job.resolved_context = context
            job.updated_at = datetime.now(timezone.utc)

    def add_module_result(self, job_id: str, result: SearchResult) -> None:
        job = self._jobs.get(job_id)
        if job:
