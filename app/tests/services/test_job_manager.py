...
class TestSetJobRunning:
    def test_transitions_status_to_running(self, job_manager, job_id):
        job_manager.set_job_running(job_id)
        assert job_manager.get_job(job_id).status == JobStatus.RUNNING

    def test_updates_updated_at(self, job_manager, job_id):
...
class TestFailJob:
    def test_transitions_status_to_failed(self, job_manager, job_id):
        job_manager.fail_job(job_id, "Something broke")
        assert job_manager.get_job(job_id).status == JobStatus.FAILED

    def test_sets_error_message(self, job_manager, job_id):
        job_manager.fail_job(job_id, "Something broke")
        assert job_manager.get_job(job_id).error == "Something broke"

    def test_sets_completed_at(self, job_manager, job_id):
        job_manager.fail_job(job_id, "error")
        assert job_manager.get_job(job_id).completed_at is not None


class TestCompleteJob:
    def test_transitions_status_to_completed(self, job_manager, job_id, assessment_result):
        job_manager.complete_job(job_id, assessment_result)
        assert job_manager.get_job(job_id).status == JobStatus.COMPLETED

    def test_stores_result(self, job_manager, job_id, assessment_result):
        job_manager.complete_job(job_id, assessment_result)
        assert job_manager.get_job(job_id).result == assessment_result

    def test_sets_completed_at(self, job_manager, job_id, assessment_result):
        job_manager.complete_job(job_id, assessment_result)
        assert job_manager.get_job(job_id).completed_at is not None
...
class TestAddModuleResult:
    ...
    def test_accumulates_multiple_results(self, job_manager, job_id):
        ...
        job_manager.add_module_result(job_id, r1)
        job_manager.add_module_result(job_id, r2)
...
class TestUpdateModuleStatus:
    def test_creates_module_status_if_not_exists(self, job_manager, job_id):
        job_manager.update_module_status(job_id, "mod_1", "Module One", ModuleStatus.PENDING)
        assert "mod_1" in job_manager.get_job(job_id).module_statuses

    def test_sets_started_at_on_running(self, job_manager, job_id):
        job_manager.update_module_status(job_id, "mod_1", "Module One", ModuleStatus.RUNNING)
        assert job_manager.get_job(job_id).module_statuses["mod_1"].started_at is not None

    def test_does_not_overwrite_started_at_on_second_running_call(self, job_manager, job_id):
        job_manager.update_module_status(job_id, "mod_1", "Module One", ModuleStatus.RUNNING)
    ...
    def test_sets_completed_at_on_completed(self, job_manager, job_id):
        job_manager.update_module_status(job_id, "mod_1", "Module One", ModuleStatus.COMPLETED)
        assert job_manager.get_job(job_id).module_statuses["mod_1"].completed_at is not None

    def test_sets_completed_at_on_failed(self, job_manager, job_id):
        job_manager.update_module_status(job_id, "mod_1", "Module One", ModuleStatus.FAILED)
        assert job_manager.get_job(job_id).module_statuses["mod_1"].completed_at is not None

    def test_stores_error_message_on_failed(self, job_manager, job_id):
        job_manager.update_module_status(
